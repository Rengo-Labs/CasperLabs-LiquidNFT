...
pub trait LiquidTransfer<Storage: ContractStorage>: ContractContext<Storage> {
    ...
    fn transfer_from_nft(&self, from: Key, to: Key, token_address: Key, token_id: U256) {
        if token_address.to_formatted_string() == data::get_kitties().to_formatted_string() {
            let (success, result): (bool, String) = runtime::call_contract(
                token_address.into_hash().unwrap_or_revert().into(),
                "transfer_from",
                runtime_args! {
                    "from" => from,
            ...
            let (buy_success, buy_result_data): (bool, String) = runtime::call_contract(
                token_address.into_hash().unwrap_or_revert().into(),
                "buy_punk",
                runtime_args! {
                    "token_id" => token_id
            ...
            let (success, result_data): (bool, String) = runtime::call_contract(
                token_address.into_hash().unwrap_or_revert().into(),
                "transfer_punk",
                runtime_args! {
                    "to" => to,
    ...
    fn onERC721Received(&self, operator: Key, from: Key, token_id: U256, data: String) // -> String
    {
